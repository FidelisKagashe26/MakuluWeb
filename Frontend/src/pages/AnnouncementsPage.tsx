...
const typeCards: TypeCard[] = [
  {
    key: "sabbath",
    title: "Sabato",
    emptyState: "Hakuna tangazo la Sabato kwa sasa."
  },
  {
    key: "ongoing",
    title: "Endelevu",
    emptyState: "Hakuna tangazo endelevu kwa sasa."
  },
  {
    key: "emergency",
    title: "Dharura",
    emptyState: "Hakuna tangazo la dharura kwa sasa."
  }
];
...
function getNewestActiveAnnouncementId(items: AnnouncementItem[], type: AnnouncementType) {
  const activeItems = items.filter((item) => item.type === type && item.status === "active");
  if (!activeItems.length) return "";

  return [...activeItems].sort((left, right) => toTs(right.createdAt) - toTs(left.createdAt))[0].id;
}
...
function getStatusRank(status: AnnouncementItem["status"]) {
...
export default function AnnouncementsPage() {
  ...
  const countsByType = useMemo(
    () =>
      allEvents.reduce<Record<AnnouncementType, number>>(
        (counts, item) => {
          counts[item.type] += 1;
          return counts;
        },
  ...
  const newestActiveId = useMemo(
    () => getNewestActiveAnnouncementId(allEvents, activeType),
    [allEvents, activeType]
  );

  const filteredItems = useMemo(() => {
    const items = allEvents.filter((item) => item.type === activeType);

    return [...items].sort((left, right) => {
      const leftNewest = left.id === newestActiveId ? 1 : 0;
      const rightNewest = right.id === newestActiveId ? 1 : 0;
      ...
      const leftAnchor = toTs(left.startDate || left.createdAt);
      const rightAnchor = toTs(right.startDate || right.createdAt);
      return rightAnchor - leftAnchor;
    });
  }, [activeType, allEvents, newestActiveId]);

  const activeTypeMeta = useMemo(
    () => typeCards.find((card) => card.key === activeType) || typeCards[0],
    [activeType]
  );
  ...
  useEffect(() => {
    ...
    if (!allEvents.some((item) => item.type === activeType)) {
      const nextType = typeCards.find((card) => countsByType[card.key] > 0)?.key;
      if (nextType) {
        setActiveType(nextType);
      }
    }
  }, [activeType, allEvents, countsByType]);
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Matangazo ya kanisa kwa aina kuu: Sabato, Endelevu na Dharura."
        />
      </Helmet>
