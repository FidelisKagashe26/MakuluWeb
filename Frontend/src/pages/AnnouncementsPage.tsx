  ...
  return (
    <>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pb-8 pt-3 sm:px-6">
              ...
              {typeCards.map((card) => {
                ...
                return (
                  <button
                    key={card.key}
                    type="button"
                    onClick={() => setActiveType(card.key)}
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[18rem_minmax(0,1fr)] xl:grid-cols-[20rem_minmax(0,1fr)]">
          <aside className="h-fit rounded-2xl border border-slate-200 bg-white/85 p-3 shadow-soft dark:border-white/10 dark:bg-slate-900/60 lg:sticky lg:top-28">
              ...
              <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft dark:border-white/10 dark:bg-slate-900/60">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <h2 className="mt-3 text-2xl font-bold text-slate-900 dark:text-white">{selectedAnnouncement.title}</h2>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <a
                      ...
                      className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-white/20 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
                    >
                      Download A4 PDF
                    </a>
                    <a
                      href={resolveAnnouncementPdfUrl(selectedAnnouncement.id, "slides")}
                      className="rounded-lg bg-church-700 px-3 py-2 text-sm font-semibold text-white transition hover:bg-church-800"
                    >
                      Download Slides PDF
                    </a>
                  </div>
                ...
                {selectedAnnouncement.documentData ? (
                  <div className="mt-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-4 text-sm text-slate-600 dark:text-slate-300">
                      <p>Start: {getDateTimeLabel(selectedAnnouncement.startDate)}</p>
                      <p>End: {getDateTimeLabel(selectedAnnouncement.endDate)}</p>
                      <p>Updated: {getDateTimeLabel(selectedAnnouncement.updatedAt)}</p>
                    </div>
                  </div>
                ) : (
                  <>
