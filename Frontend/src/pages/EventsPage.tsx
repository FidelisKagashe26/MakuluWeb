...
import { fetchPublicEvents, type EventItem, type EventStatus } from "@/services/adminService";
...
const statusCards: StatusCard[] = [
  {
    key: "ongoing",
    title: "Yanayoendelea",
    description: "Matukio yanayoendelea kwa sasa.",
    emptyState: "Hakuna tukio linaloendelea kwa sasa."
  },
  {
    key: "upcoming",
    title: "Yajayo",
    description: "Matukio yaliyopangwa kuja mbele.",
    emptyState: "Hakuna tukio la yajayo kwa sasa."
  },
  {
    key: "past",
    title: "Yaliyopita",
    description: "Matukio yaliyopita na kumbukumbu zake.",
...
function getStatusTone(status: EventStatus) {
  ...
  if (status === "past") return "bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-300";
...
export default function EventsPage() {
  const [activeStatus, setActiveStatus] = useState<EventStatus>("ongoing");
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Matukio ya kanisa: yajayo, yanayoendelea, na yaliyopita." />
      </Helmet>
        ...
        <div className="pb-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{activeStatusMeta.description}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {statusCards.map((card) => {
                const active = activeStatus === card.key;
                const hasLive = card.key === "ongoing" && countsByStatus.ongoing > 0;
                return (
                  <button
                    key={card.key}
                    type="button"
            ...
            {selectedEvent ? (
              <article className="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-soft dark:border-white/10 dark:bg-slate-900/60 sm:p-5">
                ...
                <div className="mt-4 grid grid-cols-1 gap-3 text-sm text-slate-600 dark:text-slate-300 sm:grid-cols-2">
                  <p>
                    <span className="font-semibold">Start:</span> {getDateTimeLabel(selectedEvent.startDate)}
                  </p>
                  <p>
                    <span className="font-semibold">End:</span> {getDateTimeLabel(selectedEvent.endDate)}
                  </p>
                </div>
              </article>
            ) : null}
